...
class QuantConv2d(nn.Conv2d):
    ...
    def __init__(self,   
        ...
        self.weight_quantizer = UniformQuantizer(**weight_quant_params)
        if input_quant_params['n_bits'] == 8:
            self.norm_quant = False
        else:
            self.norm_quant = True

        if self.norm_quant:
            obsever_quant_params = deepcopy(input_quant_params)
            obsever_quant_params['channel_wise'] = True
    ...
    def forward(self, x):
        ...
        if self.use_input_quant:
            ...
            if self.norm_quant:
                ...
                if len(x.shape) == 4:
                    pad0 = (self.padding,) * 2 if isinstance(self.padding, int) else self.padding
                    pad1 = tuple(x for x in reversed(pad0) for _ in range(2))
                    x = F.pad(x, pad = pad1, mode='constant', value=0)
                    ori_padding = self.padding
                    padding1 = tuple([0] * len(self.padding)) if isinstance(self.padding, tuple) else 0
                    B, C, H, W = x.shape
                if self.input_quantizer.inited == False:
                    ...
                    _ = self.input_quantizer_obs((x).reshape(B, C, -1).transpose(1,2).contiguous())
                    ...
                    w = self.weight * self.r.reshape(1,-1,1,1)
                    bias = (w * (self.b.view(1, -1, 1, 1) * torch.ones_like(w))).view(w.shape[0], -1).sum(dim=1)
                    self.bias_x = bias
                ...
                x = x/self.r.reshape(1,-1,1,1) - self.b.reshape(1,-1,1,1)
                x = self.input_quantizer(x)
                w = self.weight_quantizer(self.weight*self.r.reshape(1,-1,1,1))
            else:
                x = self.input_quantizer(x)       
        ...
        if self.use_weight_quant:
            ...
            w = self.weight
        if self.norm_quant and self.use_input_quant:
            if self.bias == None:
                out = F.conv2d(
                    x, 
                    w, 
                    torch.tensor(self.bias_x, dtype=torch.float32).to(x.device), 
                    self.stride, 
                    padding1, 
                    self.dilation, 
                    self.groups
