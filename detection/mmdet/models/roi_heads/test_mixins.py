    ...
    def simple_test_mask(self,
        ...
        if isinstance(det_bboxes, list):
            ...
            for i, (bbox, label) in enumerate(zip(det_bboxes, det_labels)):
                ...
                if det_bboxes[i].size(1) == 4:
                    #  bboxes may have 4 columns.
                    zeros_column = torch.zeros(det_bboxes[i].size(0), 1, dtype=det_bboxes[i].dtype, device=det_bboxes[i].device)
                    det_bboxes[i] = torch.cat([det_bboxes[i], zeros_column], dim=1)
